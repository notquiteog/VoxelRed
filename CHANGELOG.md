...
Adds the independent BW battle sprite provider: static full-body front/back art, not HGSS and not animated. Select ROM to restore native battle sprites. This cart targets FireRed; the mods also support LeafGreen.
...
All five core mods now load on FireRed. Dramatic Ride provides native ground riding, Surf and map-local flight. The mods also support LeafGreen; this cart retains FireRed as its base. No new sprite mod is included.
...
First Battle Art-only FireRed preview. See README for supported scope.
